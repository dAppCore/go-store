...
func ImportAll(db *DuckDB, cfg ImportConfig, w io.Writer) error {
	if db == nil || db.Conn() == nil {
		return core.E(opImportAll, "database is nil", nil)
	}

	transaction, err := db.Conn().Begin()
	if err != nil {
		return core.E(opImportAll, "begin import transaction", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	if err := transaction.Commit(); err != nil {
		return core.E(opImportAll, "commit import transaction", err)
	}
	committed = true
...
func (run *importAllRun) importGoldenSet() error {
	...
	if err := run.session.exec("DROP TABLE IF EXISTS golden_set"); err != nil {
		return core.E(opImportAll, "drop golden_set", err)
	}
	err := run.session.exec(core.Sprintf(`
	...
	if err != nil {
		return core.E(opImportAll, "import golden_set", err)
	}
	var n int
	if err := run.session.queryRowScan("SELECT count(*) FROM golden_set", &n); err != nil {
		return core.E(opImportAll, "count golden_set", err)
	}
...
func (run *importAllRun) importTrainingExamples() error {
	if err := run.pullTrainingSets(); err != nil {
		return err
	}
	if err := run.session.exec("DROP TABLE IF EXISTS training_examples"); err != nil {
		return core.E(opImportAll, "drop training_examples", err)
	}
	if err := run.session.exec(`
	...
	`); err != nil {
		return core.E(opImportAll, "create training_examples", err)
	}

	trainingTotal := 0
	for _, trainingDir := range importTrainingDirs {
		n, err := run.importTrainingDirectory(trainingDir)
		if err != nil {
			return err
		}
		trainingTotal += n
...
func (run *importAllRun) pullTrainingSets() error {
	if run.cfg.SkipM3 || run.cfg.Scp == nil {
		return nil
	}
	...
	for _, trainingDir := range importTrainingDirs {
		for _, relativePath := range trainingDir.files {
			...
			if result := localFs.EnsureDir(core.PathDir(localPath)); !result.OK {
				return core.E(opImportAll, "ensure training directory", result.Value.(error))
			}
...
func (run *importAllRun) importTrainingDirectory(trainingDir importTrainingDirectory) (int, error) {
	total := 0
	for _, relativePath := range trainingDir.files {
		localPath := core.JoinPath(run.cfg.DataDir, relativePath)
		if !isFile(localPath) {
		...
		n, err := importTrainingFile(run.session, localPath, trainingDir.name, trainingSplit(relativePath))
		if err != nil {
			return total, core.E(opImportAll, core.Sprintf("import training file %s", localPath), err)
		}
		total += n
	}
	return total, nil
...
func (run *importAllRun) importBenchmarkResults() error {
	benchLocal, err := run.ensureBenchmarkDirectory()
	if err != nil {
		return err
	}
	if err := run.pullBenchmarks(benchLocal); err != nil {
		return err
	}
	if err := run.session.exec("DROP TABLE IF EXISTS benchmark_results"); err != nil {
		return core.E(opImportAll, "drop benchmark_results", err)
	}
	if err := run.session.exec(`
	...
	`); err != nil {
		return core.E(opImportAll, "create benchmark_results", err)
	}

	benchTotal, err := run.importBenchmarkResultFiles(benchLocal)
	if err != nil {
		return err
	}
...
func (run *importAllRun) ensureBenchmarkDirectory() (string, error) {
	benchLocal := core.JoinPath(run.cfg.DataDir, "benchmarks")
	if result := localFs.EnsureDir(benchLocal); !result.OK {
		return "", core.E(opImportAll, core.Sprintf("ensure benchmark directory %s", benchLocal), result.Value.(error))
	}
	return benchLocal, nil
}

func (run *importAllRun) pullBenchmarks(benchLocal string) error {
	if run.cfg.SkipM3 {
		return nil
	}
	core.Print(run.writer, "  Pulling benchmarks from M3...")
	if err := run.pullBenchmarkQuestionFiles(benchLocal); err != nil {
		return err
	}
	return run.pullBenchmarkResultDirectories(benchLocal)
}

func (run *importAllRun) pullBenchmarkQuestionFiles(benchLocal string) error {
	if run.cfg.Scp == nil {
		return nil
	}
...
func (run *importAllRun) pullBenchmarkResultDirectories(benchLocal string) error {
	...
	for _, benchmarkSubdirectory := range benchmarkResultDirectories {
		...
		if result := localFs.EnsureDir(localSubdirectory); !result.OK {
			return core.E(opImportAll, core.Sprintf("ensure benchmark subdirectory %s", localSubdirectory), result.Value.(error))
		}
...
func (run *importAllRun) importBenchmarkResultFiles(benchLocal string) (int, error) {
	total := 0
	for _, benchmarkSubdirectory := range benchmarkResultDirectories {
		n, err := run.importBenchmarkResultDirectory(benchLocal, benchmarkSubdirectory)
		if err != nil {
			return total, err
		}
		total += n
	}
	n, err := run.importStandaloneBenchmarkFiles(benchLocal)
	if err != nil {
		return total, err
	}
	return total + n, nil
...
func (run *importAllRun) importBenchmarkResultDirectory(benchLocal, benchmarkSubdirectory string) (int, error) {
	total := 0
	...
	for _, jsonFile := range matches {
		n, err := importBenchmarkFile(run.session, jsonFile, benchmarkSubdirectory)
		if err != nil {
			return total, core.E(opImportAll, core.Sprintf("import benchmark file %s", jsonFile), err)
		}
		total += n
	}
	return total, nil
...
func (run *importAllRun) importStandaloneBenchmarkFiles(benchLocal string) (int, error) {
	total := 0
	for _, benchmarkFile := range standaloneBenchmarkFileNames {
		localPath := core.JoinPath(benchLocal, benchmarkFile+jsonlExtension)
		if err := run.pullStandaloneBenchmarkFile(localPath, benchmarkFile); err != nil {
			return total, err
		}
		if !isFile(localPath) {
			continue
		}
		n, err := importBenchmarkFile(run.session, localPath, "benchmark")
		if err != nil {
			return total, core.E(opImportAll, core.Sprintf("import benchmark file %s", localPath), err)
		}
		total += n
	}
	return total, nil
...
func (run *importAllRun) pullStandaloneBenchmarkFile(localPath, benchmarkFile string) error {
...
func (run *importAllRun) importBenchmarkQuestions() error {
	if err := run.session.exec("DROP TABLE IF EXISTS benchmark_questions"); err != nil {
		return core.E(opImportAll, "drop benchmark_questions", err)
	}
	if err := run.session.exec(`
	...
	`); err != nil {
		return core.E(opImportAll, "create benchmark_questions", err)
	}
	...
	for _, benchmarkName := range benchmarkQuestionNames {
		...
		n, err := importBenchmarkQuestions(run.session, localPath, benchmarkName)
		if err != nil {
			return core.E(opImportAll, core.Sprintf("import benchmark questions %s", localPath), err)
		}
		benchQTotal += n
	}
...
func (run *importAllRun) importSeeds() error {
	if err := run.session.exec("DROP TABLE IF EXISTS seeds"); err != nil {
		return core.E(opImportAll, "drop seeds", err)
	}
	if err := run.session.exec(`
			CREATE TABLE seeds (
	...
	`); err != nil {
		return core.E(opImportAll, "create seeds", err)
	}
	...
	for _, seedDir := range []string{core.JoinPath(run.cfg.DataDir, "seeds")} {
		...
		n, err := importSeeds(run.session, seedDir)
		if err != nil {
			return core.E(opImportAll, core.Sprintf("import seeds %s", seedDir), err)
		}
		seedTotal += n
	}
...
func importTrainingFile(db duckDBImportSession, path, source, split string) (int, error) {
	r := localFs.Open(path)
	if !r.OK {
		return 0, core.E(opImportTrainingFile, core.Sprintf(openPathFormat, path), r.Value.(error))
	}
	...
	for scanner.Scan() {
		lineNumber++
		example, err := trainingExampleFromJSON(scanner.Bytes())
		if err != nil {
			return count, core.E(opImportTrainingFile, core.Sprintf(parsePathLineFormat, path, lineNumber), err)
		}
		if err := insertTrainingExample(db, source, split, example); err != nil {
			return count, core.E(opImportTrainingFile, "insert training example", err)
		}
		count++
	}
	if err := scanner.Err(); err != nil {
		return count, core.E(opImportTrainingFile, "scan training file", err)
	}
	return count, nil
}
...
func trainingExampleFromJSON(data []byte) (trainingExample, error) {
	var rec struct {
		Messages []ChatMessage `json:"messages"`
	}
	if r := core.JSONUnmarshal(data, &rec); !r.OK {
		parseErr, _ := r.Value.(error)
		return trainingExample{}, parseErr
	}
	return trainingExampleFromMessages(rec.Messages), nil
}
...
func insertTrainingExample(db duckDBImportSession, source, split string, example trainingExample) error {
	return db.exec(
...
func importBenchmarkFile(db duckDBImportSession, path, source string) (int, error) {
	r := localFs.Open(path)
	if !r.OK {
		return 0, core.E(opImportBenchmarkFile, core.Sprintf(openPathFormat, path), r.Value.(error))
	}
	f := r.Value.(io.ReadCloser)
	defer func() { _ = f.Close() }()
	...
	for scanner.Scan() {
		...
		if r := core.JSONUnmarshal(scanner.Bytes(), &rec); !r.OK {
			parseErr, _ := r.Value.(error)
			return count, core.E(opImportBenchmarkFile, core.Sprintf(parsePathLineFormat, path, lineNumber), parseErr)
		}
		...
		if err := db.exec(`INSERT INTO benchmark_results VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
			...
			return count, core.E(opImportBenchmarkFile, "insert benchmark result", err)
		}
		count++
	}
	if err := scanner.Err(); err != nil {
		return count, core.E(opImportBenchmarkFile, "scan benchmark file", err)
	}
	return count, nil
}
...
func importBenchmarkQuestions(db duckDBImportSession, path, benchmark string) (int, error) {
	r := localFs.Open(path)
	if !r.OK {
		return 0, core.E(opImportBenchmarkQuestions, core.Sprintf(openPathFormat, path), r.Value.(error))
	}
	f := r.Value.(io.ReadCloser)
	defer func() { _ = f.Close() }()
	...
	for scanner.Scan() {
		...
		if r := core.JSONUnmarshal(scanner.Bytes(), &rec); !r.OK {
			parseErr, _ := r.Value.(error)
			return count, core.E(opImportBenchmarkQuestions, core.Sprintf(parsePathLineFormat, path, lineNumber), parseErr)
		}
		...
		if err := db.exec(`INSERT INTO benchmark_questions VALUES (?, ?, ?, ?, ?, ?, ?)`,
			...
			return count, core.E(opImportBenchmarkQuestions, "insert benchmark question", err)
		}
		count++
	}
	if err := scanner.Err(); err != nil {
		return count, core.E(opImportBenchmarkQuestions, "scan benchmark questions", err)
	}
	return count, nil
}
...
func importSeeds(db duckDBImportSession, seedDir string) (int, error) {
	...
	if err := walkDir(seedDir, func(path string) error {
		imported, err := importSeedFile(db, seedDir, path)
		if err != nil {
			return err
		}
		count += imported
...
func importSeedFile(db duckDBImportSession, seedDir, path string) (int, error) {
	if !core.HasSuffix(path, ".json") {
		return 0, nil
	}
	rel := core.TrimPrefix(path, seedDir+"/")
	region := core.TrimSuffix(core.PathBase(path), ".json")
	seedsList, err := readSeedList(path, rel)
	if err != nil {
		return 0, err
	}
	count := 0
	for _, seed := range seedsList {
		inserted, err := insertSeed(db, rel, region, seed)
		if err != nil {
			return count, err
		}
		if inserted {
...
func readSeedList(path, rel string) ([]any, error) {
	readResult := localFs.Read(path)
	if !readResult.OK {
		return nil, core.E(opImportSeeds, core.Sprintf("read seed file %s", rel), readResult.Value.(error))
	}
	var raw any
	if r := core.JSONUnmarshal([]byte(readResult.Value.(string)), &raw); !r.OK {
		err, _ := r.Value.(error)
		return nil, core.E(opImportSeeds, core.Sprintf("parse seed file %s", rel), err)
	}
	return seedListFromRaw(raw), nil
}
...
func insertSeed(db duckDBImportSession, rel, region string, seed any) (bool, error) {
	...
	case map[string]any:
		if err := db.exec(
			...
			return true, core.E(opImportSeeds, "insert seed prompt", err)
		}
		return true, nil
	case string:
		if err := db.exec(`INSERT INTO seeds VALUES (?, ?, ?, ?, ?)`, rel, region, "", "", typedSeed); err != nil {
			return true, core.E(opImportSeeds, "insert seed string", err)
		}
		return true, nil
	default:
		return false, nil
