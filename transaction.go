...
func (storeInstance *Store) Transaction(operation func(*StoreTransaction) error) error {
	if err := storeInstance.ensureReady(opTransaction); err != nil {
		return err
	}
	if operation == nil {
		return core.E(opTransaction, "operation is nil", nil)
	}

	transaction, err := storeInstance.sqliteDatabase.Begin()
	if err != nil {
		return core.E(opTransaction, "begin transaction", err)
	}
	...
	if err := operation(storeTransaction); err != nil {
		return core.E(opTransaction, "execute transaction", err)
	}
	if err := transaction.Commit(); err != nil {
		return core.E(opTransaction, "commit transaction", err)
	}
	committed = true
...
func (storeTransaction *StoreTransaction) Get(group, key string) (string, error) {
	if err := storeTransaction.ensureReady(opTransactionGet); err != nil {
		return "", err
	}
	...
	if err == sql.ErrNoRows {
		return "", core.E(opTransactionGet, core.Concat(group, "/", key), NotFoundError)
	}
	if err != nil {
		return "", core.E(opTransactionGet, "query row", err)
	}
	if expiresAt.Valid && expiresAt.Int64 <= time.Now().UnixMilli() {
		if err := storeTransaction.Delete(group, key); err != nil {
			return "", core.E(opTransactionGet, "delete expired row", err)
		}
		return "", core.E(opTransactionGet, core.Concat(group, "/", key), NotFoundError)
	}
	return value, nil
...
func (storeTransaction *StoreTransaction) Delete(group, key string) error {
	if err := storeTransaction.ensureReady(opTransactionDelete); err != nil {
		return err
	}
	...
	if err != nil {
		return core.E(opTransactionDelete, "delete row", err)
	}
	deletedRows, rowsAffectedError := deleteResult.RowsAffected()
	if rowsAffectedError != nil {
		return core.E(opTransactionDelete, "count deleted rows", rowsAffectedError)
	}
	if deletedRows > 0 {
...
func (storeTransaction *StoreTransaction) DeleteGroup(group string) error {
	if err := storeTransaction.ensureReady(opTransactionDeleteGroup); err != nil {
		return err
	}
	...
	if err != nil {
		return core.E(opTransactionDeleteGroup, "delete group", err)
	}
	deletedRows, rowsAffectedError := deleteResult.RowsAffected()
	if rowsAffectedError != nil {
		return core.E(opTransactionDeleteGroup, "count deleted rows", rowsAffectedError)
	}
	if deletedRows > 0 {
...
func (storeTransaction *StoreTransaction) DeletePrefix(groupPrefix string) error {
	if err := storeTransaction.ensureReady(opTransactionDeletePrefix); err != nil {
		return err
	}
	...
	if err != nil {
		return core.E(opTransactionDeletePrefix, "list groups", err)
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&groupName); err != nil {
			return core.E(opTransactionDeletePrefix, "scan group name", err)
		}
		groupNames = append(groupNames, groupName)
	}
	if err := rows.Err(); err != nil {
		return core.E(opTransactionDeletePrefix, "iterate groups", err)
	}
	for _, groupName := range groupNames {
		if err := storeTransaction.DeleteGroup(groupName); err != nil {
			return core.E(opTransactionDeletePrefix, "delete group", err)
		}
	}
	return nil
}
...
func (storeTransaction *StoreTransaction) GetPage(group string, offset, limit int) ([]KeyValue, error) {
	if err := storeTransaction.ensureReady(opTransactionGetPage); err != nil {
		return nil, err
	}
	if offset < 0 {
		return nil, core.E(opTransactionGetPage, "offset must be zero or positive", nil)
	}
	if limit < 0 {
		return nil, core.E(opTransactionGetPage, "limit must be zero or positive", nil)
	}
	...
	if err != nil {
		return nil, core.E(opTransactionGetPage, "query rows", err)
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&entry.Key, &entry.Value); err != nil {
			return nil, core.E(opTransactionGetPage, "scan row", err)
		}
		page = append(page, entry)
	}
	if err := rows.Err(); err != nil {
		return nil, core.E(opTransactionGetPage, rowsIterationMessage, err)
	}
	return page, nil
}
...
func (storeTransaction *StoreTransaction) AllSeq(group string) iter.Seq2[KeyValue, error] {
	return func(yield func(KeyValue, error) bool) {
		if err := storeTransaction.ensureReady(opTransactionAll); err != nil {
			yield(KeyValue{}, err)
			return
		}
...
func (storeTransaction *StoreTransaction) GroupsSeq(groupPrefix ...string) iter.Seq2[string, error] {
	...
	return func(yield func(string, error) bool) {
		if err := storeTransaction.ensureReady(opTransactionGroupsSeq); err != nil {
			yield("", err)
			return
		}
...
func (storeTransaction *StoreTransaction) Render(templateSource, group string) (string, error) {
	if err := storeTransaction.ensureReady(opTransactionRender); err != nil {
		return "", err
	}
	...
	for entry, err := range storeTransaction.All(group) {
		if err != nil {
			return "", core.E(opTransactionRender, "iterate rows", err)
		}
	...
	if err != nil {
		return "", core.E(opTransactionRender, "parse template", err)
	}
	builder := core.NewBuilder()
	if err := renderTemplate.Execute(builder, templateData); err != nil {
		return "", core.E(opTransactionRender, "execute template", err)
	}
