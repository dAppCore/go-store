...
func TestScope_Quota_Good_MaxKeys(t *testing.T) {
	...
	assertError(t, err)
	assertTruef(t, errIs(errorValue(err), QuotaExceededError), "expected QuotaExceededError, got: %v", err)
}
...
func TestScope_NewScoped_Ugly(t *T) {
	scopedStore := NewScoped(fixtureStore(t), testTenant42)
	AssertNotNil(t, scopedStore)
...
func TestScope_NewScopedConfigured_Ugly(t *T) {
	scopedStore, err := NewScopedConfigured(fixtureStore(t), ScopedStoreConfig{Namespace: testTenantA, Quota: QuotaConfig{MaxKeys: 1, MaxGroups: 1}})
...
func TestScope_NewScopedWithQuota_Ugly(t *T) {
	scopedStore, err := NewScopedWithQuota(fixtureStore(t), testTenantA, QuotaConfig{MaxGroups: 1})
	RequireNoError(t, err)
	AssertEqual(t, 1, scopedStore.Config().Quota.MaxGroups)
}

func TestScope_ScopedStore_Namespace_Good(t *T) {
	scopedStore := fixtureScopedStore(t)
...
func TestScope_ScopedStore_Namespace_Ugly(t *T) {
	scopedStore := fixtureQuotaScopedStore(t, 1, 1)
	namespace := scopedStore.Namespace()
	AssertEqual(t, testTenantA, namespace)
}

func TestScope_ScopedStore_Config_Good(t *T) {
	scopedStore := fixtureQuotaScopedStore(t, 2, 1)
	config := scopedStore.Config()
...
func TestScope_ScopedStore_Set_Good(t *T) {
	...
	AssertNoError(t, err)
	AssertTrue(t, fixtureScopedExists(t, scopedStore, "colour"))
}
...
func TestScope_ScopedStore_Set_Ugly(t *T) {
	scopedStore := fixtureQuotaScopedStore(t, 1, 0)
	RequireNoError(t, scopedStore.Set("colour", "blue"))
...
func TestScope_ScopedStore_SetIn_Good(t *T) {
	...
	AssertNoError(t, err)
	AssertTrue(t, fixtureScopedExistsIn(t, scopedStore, "config", "colour"))
}
...
func TestScope_ScopedStore_SetIn_Ugly(t *T) {
	scopedStore := fixtureQuotaScopedStore(t, 0, 1)
	RequireNoError(t, scopedStore.SetIn("config", "colour", "blue"))
...
func TestScope_ScopedStore_SetWithTTL_Good(t *T) {
	...
	AssertNoError(t, err)
	AssertTrue(t, fixtureScopedExistsIn(t, scopedStore, "sessions", "token"))
}
...
func TestScope_ScopedStore_Delete_Good(t *T) {
	...
	AssertNoError(t, err)
	AssertFalse(t, fixtureScopedExistsIn(t, scopedStore, "config", "colour"))
}
...
func TestScope_ScopedStore_Delete_Ugly(t *T) {
	...
	AssertNoError(t, err)
	AssertFalse(t, fixtureScopedExistsIn(t, scopedStore, "missing", "key"))
}
...
func TestScope_ScopedStore_DeleteGroup_Ugly(t *T) {
	...
	AssertNoError(t, err)
	AssertFalse(t, fixtureScopedGroupExists(t, scopedStore, "missing"))
}
...
func TestScope_ScopedStore_DeletePrefix_Good(t *T) {
	...
	AssertNoError(t, err)
	AssertFalse(t, fixtureScopedGroupExists(t, scopedStore, testCacheA))
}
...
func TestScope_ScopedStore_DeletePrefix_Ugly(t *T) {
	...
	AssertNoError(t, err)
	AssertFalse(t, fixtureScopedGroupExists(t, scopedStore, "cache"))
}
...
func TestScope_ScopedStore_All_Good(t *T) {
	scopedStore := fixtureScopedStore(t)
	RequireNoError(t, scopedStore.SetIn("config", "colour", "blue"))
	entries, err := fixtureCollectKeyValues(scopedStore.All("config"))
	AssertNoError(t, err)
...
func TestScope_ScopedStore_All_Ugly(t *T) {
	scopedStore := fixtureScopedStore(t)
	entries, err := fixtureCollectKeyValues(scopedStore.All("missing"))
	AssertNoError(t, err)
	AssertEmpty(t, entries)
}
...
func TestScope_ScopedStore_AllSeq_Good(t *T) {
	scopedStore := fixtureScopedStore(t)
	RequireNoError(t, scopedStore.SetIn("config", "colour", "blue"))
	entries, err := fixtureCollectKeyValues(scopedStore.AllSeq("config"))
	AssertNoError(t, err)
...
func TestScope_ScopedStore_AllSeq_Ugly(t *T) {
	scopedStore := fixtureScopedStore(t)
	entries, err := fixtureCollectKeyValues(scopedStore.AllSeq("missing"))
	AssertNoError(t, err)
	AssertEmpty(t, entries)
}
...
func TestScope_ScopedStore_GroupsSeq_Good(t *T) {
	scopedStore := fixtureScopedStore(t)
	RequireNoError(t, scopedStore.SetIn("config", "colour", "blue"))
	groups, err := fixtureCollectGroups(scopedStore.GroupsSeq())
	AssertNoError(t, err)
	AssertEqual(t, []string{"config"}, groups)
}
...
func TestScope_ScopedStore_GroupsSeq_Ugly(t *T) {
	scopedStore := fixtureScopedStore(t)
	groups, err := fixtureCollectGroups(scopedStore.GroupsSeq("missing"))
	AssertNoError(t, err)
	AssertEmpty(t, groups)
}
...
func TestScope_ScopedStore_Transaction_Good(t *T) {
	scopedStore := fixtureScopedStore(t)
	err := scopedStore.Transaction(func(transaction *ScopedStoreTransaction) core.Result { return transaction.Set("colour", "blue") })
	AssertNoError(t, err)
	AssertTrue(t, fixtureScopedExists(t, scopedStore, "colour"))
}
...
func TestScope_ScopedStore_Transaction_Bad(t *T) {
	...
	AssertError(t, err)
	AssertFalse(t, fixtureScopedExists(t, scopedStore, "colour"))
}

func TestScope_ScopedStore_Transaction_Ugly(t *T) {
	scopedStore := fixtureScopedStore(t)
	err := scopedStore.Transaction(func(transaction *ScopedStoreTransaction) core.Result { return core.Fail(NewError("rollback")) })
	AssertError(t, err)
	AssertFalse(t, fixtureScopedExists(t, scopedStore, "colour"))
}
