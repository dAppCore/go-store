...
func assertNoError(t testing.TB, err any) {
	t.Helper()
	if hasError(err) {
		t.Fatalf("unexpected error: %v", errorText(err))
	}
}

func assertNoErrorf(t testing.TB, err any, format string, args ...any) {
	t.Helper()
	if hasError(err) {
		t.Fatalf("unexpected error: %v — "+format, append([]any{errorText(err)}, args...)...)
	}
}

func assertError(t testing.TB, err any) {
	t.Helper()
	if !hasError(err) {
...
func assertErrorIs(t testing.TB, err any, target error) {
	t.Helper()
	actual := errorValue(err)
	if !errIs(actual, target) {
...
func errorValue(value any) error {
	switch typed := value.(type) {
	case nil:
		return nil
	case core.Result:
