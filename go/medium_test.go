...
func (medium *memoryMedium) Read(path string) (string, core.Result) {
	medium.lock.Lock()
	defer medium.lock.Unlock()
	content, ok := medium.files[path]
	if !ok {
		return "", core.Fail(core.E("memoryMedium.Read", testFileNotFoundPrefix+path, nil))
	}
	return content, core.Ok(nil)
}

func (medium *memoryMedium) Write(path, content string) core.Result {
	medium.lock.Lock()
	defer medium.lock.Unlock()
...
func (medium *memoryMedium) WriteMode(path, content string, _ fs.FileMode) core.Result {
	return medium.Write(path, content)
}

func (medium *memoryMedium) EnsureDir(string) core.Result { return core.Ok(nil) }

func (medium *memoryMedium) Create(path string) (goio.WriteCloser, core.Result) {
	return &memoryWriter{medium: medium, path: path, buffer: core.NewBuilder()}, core.Ok(nil)
}

func (medium *memoryMedium) Append(path string) (goio.WriteCloser, core.Result) {
	medium.lock.Lock()
	defer medium.lock.Unlock()
	buffer := core.NewBuilder()
	buffer.WriteString(medium.files[path])
	return &memoryWriter{medium: medium, path: path, buffer: buffer}, core.Ok(nil)
}

func (medium *memoryMedium) ReadStream(path string) (goio.ReadCloser, core.Result) {
	medium.lock.Lock()
	defer medium.lock.Unlock()
	return goio.NopCloser(core.NewReader(medium.files[path])), core.Ok(nil)
}

func (medium *memoryMedium) WriteStream(path string) (goio.WriteCloser, core.Result) {
...
func (medium *memoryMedium) Delete(path string) core.Result {
	medium.lock.Lock()
	defer medium.lock.Unlock()
...
func (medium *memoryMedium) DeleteAll(path string) core.Result {
	medium.lock.Lock()
	defer medium.lock.Unlock()
...
func (medium *memoryMedium) Rename(oldPath, newPath string) core.Result {
	medium.lock.Lock()
	defer medium.lock.Unlock()
	content, ok := medium.files[oldPath]
	if !ok {
		return core.Fail(core.E("memoryMedium.Rename", testFileNotFoundPrefix+oldPath, nil))
	}
...
func (medium *renameFailMedium) Rename(string, string) core.Result {
	return core.Fail(core.E("renameFailMedium.Rename", "forced rename failure", nil))
}
...
func (medium *writeFailOnceMedium) Write(path, content string) core.Result {
	if medium.failures > 0 {
		medium.failures--
		return core.Fail(core.E("writeFailOnceMedium.Write", "forced write failure", nil))
	}
	return medium.memoryMedium.Write(path, content)
}

func (medium *memoryMedium) List(path string) ([]fs.DirEntry, core.Result) { return nil, core.Ok(nil) }

func (medium *memoryMedium) Stat(path string) (fs.FileInfo, core.Result) {
	if !medium.Exists(path) {
		return nil, core.Fail(core.E("memoryMedium.Stat", testFileNotFoundPrefix+path, nil))
	}
	return fileInfoStub{name: core.PathBase(path)}, core.Ok(nil)
}

func (medium *memoryMedium) Open(path string) (fs.File, core.Result) {
	if !medium.Exists(path) {
		return nil, core.Fail(core.E("memoryMedium.Open", testFileNotFoundPrefix+path, nil))
	}
	return newMemoryFile(path, medium.files[path]), core.Ok(nil)
}
...
func (writer *memoryWriter) Close() error {
	...
	writer.closed = true
	result := writer.medium.Write(writer.path, writer.buffer.String())
	if result.OK {
		return nil
	}
...
func newMemoryFile(name, content string) *memoryFile {
	return &memoryFile{reader: core.NewReader(content), name: name}
}

func (file *memoryFile) Read(p []byte) (int, error) {
...
func TestMedium_WithMedium_Good(t *testing.T) {
	...
	r := New(testMemoryDatabasePath, WithMedium(medium))
	assertNoError(t, r)
...
func TestMedium_WithMedium_Good_PersistsDatabaseThroughMedium(t *testing.T) {
	...
	r := New(testAppDatabaseFile, WithMedium(medium))
	assertNoError(t, r)
	storeInstance := r.Value.(*Store)
...
func TestMedium_Compact_Good_MediumRoutesArchive(t *testing.T) {
	...
	medium := newMemoryMedium()
	r := New(testMemoryDatabasePath, WithJournal(testJournalEndpoint, "core", "events"), WithMedium(medium))
	assertNoError(t, r)
	storeInstance := r.Value.(*Store)
	defer func() { _ = storeInstance.Close() }()
...
func TestMedium_WithMedium_Bad(t *T) {
	r := New(testMemoryDatabasePath, WithMedium(nil))
	RequireNoError(t, r)
	storeInstance := r.Value.(*Store)
	defer func() { _ = storeInstance.Close() }()
	AssertNil(t, storeInstance.Medium())
...
func TestMedium_WithMedium_Ugly(t *T) {
	option := WithMedium(newFixtureMedium())
