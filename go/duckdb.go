...
//
//	defer func() { _ = db.Close() }()
func (db *DuckDB) Close() core.Result {
...
func (db *DuckDB) Exec(query string, args ...any) core.Result {
	_, err := db.conn.Exec(query, args...)
	if err != nil {
		return core.Fail(core.E("store.DuckDB.Exec", "execute query", err))
	}
	return core.Ok(nil)
}
...
func (db *DuckDB) QueryRowScan(query string, dest any, args ...any) core.Result {
	if err := db.conn.QueryRow(query, args...).Scan(dest); err != nil {
		return core.Fail(err)
	}
	return core.Ok(nil)
}
...
//
//	rows, err := db.QueryGoldenSet(500)
func (db *DuckDB) QueryGoldenSet(minChars int) ([]GoldenSetRow, core.Result) {
	...
	if err != nil {
		return nil, core.Fail(core.E("store.DuckDB.QueryGoldenSet", "query golden_set", err))
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.Idx, &r.SeedID, &r.Domain, &r.Voice,
			...
			return nil, core.Fail(core.E("store.DuckDB.QueryGoldenSet", "scan golden_set row", err))
		}
		result = append(result, r)
	}
	if err := rows.Err(); err != nil {
		return nil, core.Fail(core.E("store.DuckDB.QueryGoldenSet", "iterate golden_set rows", err))
	}
...
func (db *DuckDB) CountGoldenSet() (int, core.Result) {
	var count int
	err := db.conn.QueryRow("SELECT COUNT(*) FROM golden_set").Scan(&count)
	if err != nil {
		return 0, core.Fail(core.E("store.DuckDB.CountGoldenSet", "count golden_set", err))
	}
...
//
//	prompts, err := db.QueryExpansionPrompts("pending", 100)
func (db *DuckDB) QueryExpansionPrompts(status string, limit int) ([]ExpansionPromptRow, core.Result) {
	...
	if err != nil {
		return nil, core.Fail(core.E("store.DuckDB.QueryExpansionPrompts", "query expansion_prompts", err))
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.Idx, &r.SeedID, &r.Region, &r.Domain,
			...
			return nil, core.Fail(core.E("store.DuckDB.QueryExpansionPrompts", "scan expansion_prompt row", err))
		}
		result = append(result, r)
	}
	if err := rows.Err(); err != nil {
		return nil, core.Fail(core.E("store.DuckDB.QueryExpansionPrompts", "iterate expansion_prompt rows", err))
	}
	return result, core.Ok(nil)
}
...
func (db *DuckDB) CountExpansionPrompts() (total int, pending int, err core.Result) {
	if scanErr := db.conn.QueryRow("SELECT COUNT(*) FROM expansion_prompts").Scan(&total); scanErr != nil {
		return 0, 0, core.Fail(core.E("store.DuckDB.CountExpansionPrompts", "count expansion_prompts", scanErr))
	}
	if scanErr := db.conn.QueryRow("SELECT COUNT(*) FROM expansion_prompts WHERE status = 'pending'").Scan(&pending); scanErr != nil {
		return total, 0, core.Fail(core.E("store.DuckDB.CountExpansionPrompts", "count pending expansion_prompts", scanErr))
	}
	return total, pending, core.Ok(nil)
}
...
func (db *DuckDB) UpdateExpansionStatus(idx int64, status string) core.Result {
	_, err := db.conn.Exec("UPDATE expansion_prompts SET status = ? WHERE idx = ?", status, idx)
	if err != nil {
		return core.Fail(core.E("store.DuckDB.UpdateExpansionStatus", core.Sprintf("update expansion_prompt %d", idx), err))
	}
	return core.Ok(nil)
}
...
func (db *DuckDB) QueryRows(query string, args ...any) ([]map[string]any, core.Result) {
	rows, err := db.conn.Query(query, args...)
	if err != nil {
		return nil, core.Fail(core.E(opDuckDBQueryRows, "query", err))
	}
	defer func() { _ = rows.Close() }()

	cols, err := rows.Columns()
	if err != nil {
		return nil, core.Fail(core.E(opDuckDBQueryRows, "columns", err))
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(ptrs...); err != nil {
			return nil, core.Fail(core.E(opDuckDBQueryRows, "scan", err))
		}
	...
	if err := rows.Err(); err != nil {
		return nil, core.Fail(core.E(opDuckDBQueryRows, "iterate", err))
	}
	return result, core.Ok(nil)
}
...
func (db *DuckDB) EnsureScoringTables() core.Result {
	if _, err := db.conn.Exec(core.Sprintf(`CREATE TABLE IF NOT EXISTS %s (
		...
		return core.Fail(core.E(opDuckDBEnsureScoringTables, "create checkpoint_scores", err))
	}
	if _, err := db.conn.Exec(core.Sprintf(`CREATE TABLE IF NOT EXISTS %s (
		...
		return core.Fail(core.E(opDuckDBEnsureScoringTables, "create probe_results", err))
	}
	if _, err := db.conn.Exec(`CREATE TABLE IF NOT EXISTS scoring_results (
		...
		return core.Fail(core.E(opDuckDBEnsureScoringTables, "create scoring_results", err))
	}
	return core.Ok(nil)
}
...
func (db *DuckDB) WriteScoringResult(model, promptID, suite, dimension string, score float64) core.Result {
	_, err := db.conn.Exec(
		`INSERT INTO scoring_results (model, prompt_id, suite, dimension, score) VALUES (?, ?, ?, ?, ?)`,
		model, promptID, suite, dimension, score,
	)
	if err != nil {
		return core.Fail(core.E("store.DuckDB.WriteScoringResult", "insert scoring result", err))
	}
	return core.Ok(nil)
}
...
//	n := counts["golden_set"]
func (db *DuckDB) TableCounts() (map[string]int, core.Result) {
