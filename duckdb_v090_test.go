...
func TestDuckdbV090_OpenDuckDBReadWrite_Good(t *T) {
...
func TestDuckdbV090_OpenDuckDBReadWrite_Bad(t *T) {
	database, err := store.OpenDuckDBReadWrite(Path(t.TempDir(), "missing", "db.duckdb"))
	AssertError(t, err)
	AssertNil(t, database)
}

func TestDuckdbV090_OpenDuckDBReadWrite_Ugly(t *T) {
...
func TestDuckdbV090_OpenDuckDB_Good(t *T) {
...
func TestDuckdbV090_OpenDuckDB_Bad(t *T) {
	reader, err := store.OpenDuckDB(Path(t.TempDir(), "missing.duckdb"))
	AssertError(t, err)
	AssertNil(t, reader)
}

func TestDuckdbV090_OpenDuckDB_Ugly(t *T) {
...
func TestDuckdbV090_DuckDB_Close_Good(t *T) {
...
func TestDuckdbV090_DuckDB_Close_Bad(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_Close_Ugly(t *T) {
	database := ax7DuckDB(t)
	RequireNoError(t, database.Close())
	err := database.Close()
	AssertNoError(t, err)
...
func TestDuckdbV090_DuckDB_Path_Good(t *T) {
	database := ax7DuckDB(t)
	path := database.Path()
	AssertContains(t, path, "ax7.duckdb")
}

func TestDuckdbV090_DuckDB_Path_Bad(t *T) {
	database := ax7DuckDB(t)
	RequireNoError(t, database.Close())
	path := database.Path()
	AssertContains(t, path, "ax7.duckdb")
...
func TestDuckdbV090_DuckDB_Path_Ugly(t *T) {
...
func TestDuckdbV090_DuckDB_Conn_Good(t *T) {
	database := ax7DuckDB(t)
	conn := database.Conn()
	AssertNotNil(t, conn)
}

func TestDuckdbV090_DuckDB_Conn_Bad(t *T) {
	database := ax7DuckDB(t)
	RequireNoError(t, database.Close())
	conn := database.Conn()
	AssertNotNil(t, conn)
...
func TestDuckdbV090_DuckDB_Conn_Ugly(t *T) {
	database := ax7DuckDB(t)
	err := database.Conn().Ping()
	AssertNoError(t, err)
}

func TestDuckdbV090_DuckDB_Exec_Good(t *T) {
	database := ax7DuckDB(t)
	err := database.Exec("CREATE TABLE ax7_exec (id INTEGER)")
	AssertNoError(t, err)
}

func TestDuckdbV090_DuckDB_Exec_Bad(t *T) {
	database := ax7DuckDB(t)
	err := database.Exec("CREATE TABLE")
	AssertError(t, err)
}

func TestDuckdbV090_DuckDB_Exec_Ugly(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_QueryRowScan_Good(t *T) {
	database := ax7DuckDB(t)
	var n int
...
func TestDuckdbV090_DuckDB_QueryRowScan_Bad(t *T) {
	database := ax7DuckDB(t)
	var n int
...
func TestDuckdbV090_DuckDB_QueryRowScan_Ugly(t *T) {
	database := ax7DuckDB(t)
	var text string
...
func TestDuckdbV090_DuckDB_QueryGoldenSet_Good(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_QueryGoldenSet_Bad(t *T) {
	database := ax7DuckDB(t)
	rows, err := database.QueryGoldenSet(1)
	AssertError(t, err)
	AssertNil(t, rows)
...
func TestDuckdbV090_DuckDB_QueryGoldenSet_Ugly(t *T) {
	database := ax7DuckDB(t)
	ax7SeedDuckDB(t, database)
...
func TestDuckdbV090_DuckDB_CountGoldenSet_Good(t *T) {
	database := ax7DuckDB(t)
	ax7SeedDuckDB(t, database)
...
func TestDuckdbV090_DuckDB_CountGoldenSet_Bad(t *T) {
	database := ax7DuckDB(t)
	count, err := database.CountGoldenSet()
	AssertError(t, err)
...
func TestDuckdbV090_DuckDB_CountGoldenSet_Ugly(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_QueryExpansionPrompts_Good(t *T) {
	database := ax7DuckDB(t)
	ax7SeedDuckDB(t, database)
...
func TestDuckdbV090_DuckDB_QueryExpansionPrompts_Bad(t *T) {
	database := ax7DuckDB(t)
	rows, err := database.QueryExpansionPrompts("pending", 10)
	AssertError(t, err)
	AssertNil(t, rows)
...
func TestDuckdbV090_DuckDB_QueryExpansionPrompts_Ugly(t *T) {
	database := ax7DuckDB(t)
	ax7SeedDuckDB(t, database)
...
func TestDuckdbV090_DuckDB_CountExpansionPrompts_Good(t *T) {
	database := ax7DuckDB(t)
	ax7SeedDuckDB(t, database)
...
func TestDuckdbV090_DuckDB_CountExpansionPrompts_Bad(t *T) {
	database := ax7DuckDB(t)
	total, pending, err := database.CountExpansionPrompts()
	AssertError(t, err)
...
func TestDuckdbV090_DuckDB_CountExpansionPrompts_Ugly(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_UpdateExpansionStatus_Good(t *T) {
	database := ax7DuckDB(t)
	ax7SeedDuckDB(t, database)
...
func TestDuckdbV090_DuckDB_UpdateExpansionStatus_Bad(t *T) {
	database := ax7DuckDB(t)
	err := database.UpdateExpansionStatus(7, "done")
	AssertError(t, err)
}

func TestDuckdbV090_DuckDB_UpdateExpansionStatus_Ugly(t *T) {
	database := ax7DuckDB(t)
	ax7SeedDuckDB(t, database)
...
func TestDuckdbV090_DuckDB_QueryRows_Good(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_QueryRows_Bad(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_QueryRows_Ugly(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_EnsureScoringTables_Good(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_EnsureScoringTables_Bad(t *T) {
	database := ax7DuckDB(t)
	RequireNoError(t, database.Close())
	err := database.EnsureScoringTables()
	AssertError(t, err)
...
func TestDuckdbV090_DuckDB_EnsureScoringTables_Ugly(t *T) {
	database := ax7DuckDB(t)
...
func TestDuckdbV090_DuckDB_WriteScoringResult_Good(t *T) {
	database := ax7DuckDB(t)
	RequireNoError(t, database.EnsureScoringTables())
...
func TestDuckdbV090_DuckDB_WriteScoringResult_Bad(t *T) {
	database := ax7DuckDB(t)
	err := database.WriteScoringResult("model", "prompt", "suite", "dimension", 0.5)
	AssertError(t, err)
}

func TestDuckdbV090_DuckDB_WriteScoringResult_Ugly(t *T) {
	database := ax7DuckDB(t)
	RequireNoError(t, database.EnsureScoringTables())
...
func TestDuckdbV090_DuckDB_TableCounts_Good(t *T) {
	database := ax7DuckDB(t)
	ax7SeedDuckDB(t, database)
...
func TestDuckdbV090_DuckDB_TableCounts_Bad(t *T) {
	database := ax7DuckDB(t)
	RequireNoError(t, database.Close())
	counts, err := database.TableCounts()
	AssertNoError(t, err)
...
func TestDuckdbV090_DuckDB_TableCounts_Ugly(t *T) {
	database := ax7DuckDB(t)
	RequireNoError(t, database.EnsureScoringTables())
	counts, err := database.TableCounts()
	AssertNoError(t, err)
